...
data "template_file" "namespace" {
  emplate = "${file("${path.module}/00-namespace.yaml")}"
...
data "template_file" "rbac" {
  emplate = "${file("${path.module}/01-rbac.yaml")}"
...
data "template_file" "limitrange" {
  emplate = "${file("${path.module}/02-limitrange.yaml")}"
...
data "template_file" "resourcequota" {
  emplate = "${file("${path.module}/03-resourcequota.yaml")}"
...
data "template_file" "networkpolicy" {
  emplate = "${file("${path.module}/04-networkpolicy.yaml")}"
